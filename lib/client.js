 ...
 * @param {String} data The content as a string.
 *
 * @return {Object|String} The parsed content, or the string
 *   if the content could not be parsed.
 */
Client.parse = function (type, data) {
  debug('#parse called for "%s"', type);

  var p = Client.parsers[type];
...
Client.prototype.receive = function (message, callback) {
  ...
  var error;
  var ctype = message.headers['content-type'];
  var encoding = mime.charset(ctype);
  var data = [];
  var size = 0;
  ...
  message.on('end', function received() {
    ...
    data = Client.parse(mime.extension(ctype), data);
