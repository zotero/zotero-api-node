...
Object.defineProperties(Message.prototype, {
  ...
  error: {
    get: function () {
      if (!this.received || this.ok) return undefined;
...
Message.prototype.send = function (client) {
  if (!this.req)
    throw new Error('message not bound to a request yet');
...
Message.prototype.retry = function (callback) {
  if (!this.client)
    throw new Error('cannot retry sending message: no associated client');
...
Message.prototype.next = function (callback) {
  ...
  if (!this.client)
    throw new Error('cannot send next message: no associated client');

  var link = this.links.next;
  callback = callback || noop;

  return this.client.get(link.path, link.options, callback);
};
...
Message.parsers = {
  json: function (data, callback) {
    try {
      callback(null, JSON.parse(data.toString(this.encoding)));
