 ...
 *
 * @return {Message} The API response message (future).
 */
Client.prototype.request = function (options, data, callback) {
  ...
  var self = this;
  var message = new Message(this, https.request(options));
  ...
  message.req
    ...
    .on('sent', this.emit.bind(this, 'sent', message));


  if (this.state.limited) {
    setTimeout(function () { message.send() }, this.state.limited);
